...
<div class="pt-3 pb-2 mb-3 border-bottom d-flex align-items-center justify-content-between">
    <h1 class="h3 m-0">Novo Produto</h1>
    <a href="{{ route('dashboard.barista.products.index') }}" class="btn btn-outline-secondary">Voltar</a>
</div>

<div class="card">
    <div class="card-body">
        <form action="{{ route('dashboard.barista.products.store') }}" method="POST" enctype="multipart/form-data" class="row g-3">
            @csrf
            <div class="col-md-6">
                ...
                <select name="bares_id" class="form-select" required>
                    ...
                    @foreach($establishments as $e)
                        <option value="{{ $e->bares_id }}" @selected(old('bares_id')==$e->bares_id)>{{ $e->nome }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                ...
                <select name="tipo_produto" id="tipo_produto" class="form-select" required>
                    ...
                    @foreach(['bebida','alimento','lanche','salgado','sobremesa','outro'] as $tp)
                        <option value="{{ $tp }}" @selected(old('tipo_produto')==$tp)>{{ ucfirst($tp) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-6" id="subtipo_wrap" style="display:none;">
                ...
                <select name="subtipo_bebida" class="form-select">
                    ...
                    @foreach(['alcoolica','nao_alcoolica','cafe','cha','outro'] as $st)
                        <option value="{{ $st }}" @selected(old('subtipo_bebida')==$st)>{{ ucfirst(str_replace('_',' ',$st)) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <select name="base_id" id="base_id" class="form-select" disabled>
            ...
            <div class="col-md-4">
                ...
                <input type="number" step="0.01" name="preco" value="{{ old('preco') }}" class="form-control">
            </div>
            <div class="col-md-4">
                <label class="form-label">Quantidade em Estoque</label>
                <input type="number" name="quantidade_estoque" value="{{ old('quantidade_estoque') }}" class="form-control">
            </div>
            <div class="col-md-4">
                <label class="form-label">Unidade</label>
                <input type="text" name="unidade" value="{{ old('unidade') }}" class="form-control" placeholder="ex.: dose, garrafa, porção">
            </div>
            <div class="col-12">
            ...
            <div class="col-12">
                ...
                <input type="text" name="tags" value="{{ old('tags') }}" class="form-control" placeholder="ex.: vodka, drink, doce">
            </div>
            <div class="col-md-4">
                <label class="form-label">Status</label>
                <select name="status" class="form-select">
                    @foreach(['ativo','inativo'] as $st)
                        <option value="{{ $st }}" @selected(old('status',$st)=='inativo')>{{ ucfirst($st) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-12">
                <label class="form-label">Imagens do Produto (1 a 6)</label>
                <input type="file" name="fotos[]" class="form-control" accept="image/*" multiple>
                <div class="form-text">Envie de 1 a 6 imagens. Processadas para 800x600px.</div>
            </div>
