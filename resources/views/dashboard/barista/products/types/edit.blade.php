...
<div class="d-flex align-items-center justify-content-between pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h3 m-0">Editar Tipo de Produto</h1>
    <a href="{{ route('dashboard.barista.products.types.index') }}" class="btn btn-outline-secondary">Voltar</a>
</div>
<div class="card"><div class="card-body">
    <form method="POST" action="{{ route('dashboard.barista.products.types.update',$row->tipo_id) }}" class="row g-3">
        @csrf @method('PUT')
        <div class="col-md-6">
            ...
            <select name="familia_id" class="form-select" required>
                ...
                @foreach($families as $f)
                    <option value="{{ $f->familia_id }}" @selected(old('familia_id',$row->familia_id)==$f->familia_id)>{{ $f->nome }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-6">
